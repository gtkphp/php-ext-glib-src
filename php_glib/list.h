...
#define PHP_G_LIST_GET_PTR(val)   php_g_list_get_ptr(val)
...
#if ZEND_DEBUG
#define PHP_G_LIST_DEBUG_FE PHP_GTK_FE(g_list_dump,          arginfo_g_list_dump)
#else
#define PHP_G_LIST_DEBUG_FE
//#error "configure with --enable-debug"
#endif
...
GList *php_g_list_get_ptr(php_g_list *val);
...
php_g_list *php_g_list_insert_sorted(php_g_list *php_list, zval *z_data, zval *z_func);
...
#if ZEND_DEBUG
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_g_list_dump, ZEND_RETURN_VALUE, 1, IS_VOID, 0)
...
ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_g_list_insert_sorted, ZEND_RETURN_VALUE, 3, GList, 1)
    ZEND_ARG_OBJ_INFO(ZEND_SEND_BY_VAL, list, GList, 0)
    ZEND_ARG_INFO(ZEND_SEND_BY_VAL, data)
    ZEND_ARG_INFO(ZEND_SEND_BY_VAL, func)
ZEND_END_ARG_INFO()
